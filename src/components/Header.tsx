...
export default function Header() {
  ...
  return (
    <motion.header 
      ...
      className={"fixed top-0 left-0 right-0 z-[100] transition-all duration-500 " + (scrolled ? 'py-4' : 'py-10')}
    >
      <div className="max-w-7xl mx-auto px-6 sm:px-12">
        <nav className="relative flex items-center justify-between">
          <Link href="/" className="group flex items-center gap-3">
             <div className="w-12 h-12 glass rounded-2xl flex items-center justify-center font-display font-black text-2xl text-white group-hover:bg-lime-400 group-hover:text-black group-hover:scale-110 transition-all duration-500 border border-white/10">
                L
             </div>
             <span className="font-display font-bold text-xl tracking-tighter hidden sm:block text-white">MUDUNURI LOKESH VARMA</span>
          </Link>
          
          {/* Desktop Menu */}
          <div className="hidden md:flex items-center gap-2 p-1.5 glass rounded-[2rem] border border-white/10">
            {navLinks.map((link) => {
              ...
              if (isMemories) {
                return (
                  <Link
                    key={link.label}
                    href={link.href}
                    className="group relative px-6 py-3 text-[10px] uppercase tracking-[0.2em] font-black text-white transition-all flex items-center gap-2"
                  >
                    ...
                    <span className="absolute bottom-1.5 left-3.5 right-3.5 h-[2.5px] rounded-full bg-gradient-to-r from-lime-400 via-emerald-400 to-lime-300 animate-neon-glow overflow-hidden transition-all duration-300 group-hover:h-[3px]">
              ...
              return (
                <Link
                  key={link.label}
                  href={link.href}
                  className="px-6 py-3 text-[10px] uppercase tracking-[0.2em] font-bold text-neutral-300 hover:text-white hover:bg-white/10 rounded-full transition-all"
                >
                  {link.label}
                </Link>
              );
          ...
          <div className="flex items-center gap-4">
             <Link 
              href="/#contact" 
              className="px-8 py-3 bg-lime-400 text-black font-black rounded-[1.5rem] text-[10px] uppercase tracking-[0.2em] hover:bg-lime-300 hover:scale-105 active:scale-95 transition-all shadow-[0_10px_30px_rgba(163,230,53,0.25)]"
            >
              Collaborate
            </Link>
